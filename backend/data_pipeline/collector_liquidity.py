...
def collect_liquidity_funnel_data() -> Dict[str, Any]:
    try:
        ...
        rsp_is_negative = rsp_1w_return < 0 and spy_20d_return > 0

        history_90d = {
            "dates": [d.strftime("%Y-%m-%d") for d in data.tail(90).index],
            "spy": [round(v, 2) for v in normalized["SPY"].tail(90).tolist()],
            "rsp": [round(v, 2) for v in normalized["RSP"].tail(90).tolist()],
            "qqq": [round(v, 2) for v in normalized["QQQ"].tail(90).tolist()],
            "dia": [round(v, 2) for v in normalized["DIA"].tail(90).tolist()],
            "iwm": [round(v, 2) for v in normalized["IWM"].tail(90).tolist()],
        }
        ...
        return {
            ...
            "rsp_is_negative_while_spy_positive": bool(rsp_is_negative),
