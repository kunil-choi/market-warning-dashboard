...
MEGA_IPO_WATCHLIST = [
    {
        "company": "SpaceX",
        "est_valuation_bn": 350,
        "status": "Considering",
        "sector": "AI/Space",
    },
    {
        "company": "OpenAI",
        "est_valuation_bn": 300,
        "status": "Considering",
        "sector": "AI",
    },
    {
...
def collect_ipo_data() -> Dict[str, Any]:
    try:
        ...
        try:
            ...
            ipo_etf_90d = {}
            for t in ["IPO", "IPOS"]:
                if t in ipo_etf_data.columns:
                    ret = (ipo_etf_data[t].iloc[-1] / ipo_etf_data[t].iloc[-min(63, len(ipo_etf_data)-1)] - 1) * 100
                    ipo_etf_90d[t] = round(float(ret), 2)
        except Exception:
            ipo_etf_90d = {"IPO": 0, "IPOS": 0}
            ...
            try:
                ...
                if len(hist) > 0:
                    ipo_performance[ticker] = {
                        ...
                        "6m_return": round((hist["Close"].iloc[-1] / hist["Close"].iloc[0] - 1) * 100, 2),
                    }
            except Exception:
                pass

        total_pipeline_bn = sum(
            c["est_valuation_bn"] for c in MEGA_IPO_WATCHLIST
        ...
        try:
            vix_data = yf.download("^VIX", period="3mo", progress=False)["Close"]
            vix_current = float(vix_data.iloc[-1])
            vix_avg_3m = float(vix_data.mean())
        ...
        return {
            ...
            "total_pipeline_bn": round(total_pipeline_bn, 0),
            "estimated_market_impact_bn": round(estimated_market_impact_bn, 0),
            "pipeline_vs_korea_gdp_ratio": round(pipeline_vs_korea_gdp, 2),
            ...
            "vix_is_extreme_low": vix_is_low,
            "ipo_heat_index": min(100, ipo_heat_index),
            "active_ipo_count": sum(1 for c in MEGA_IPO_WATCHLIST if c["status"] == "Filed"),
            "considering_ipo_count": sum(1 for c in MEGA_IPO_WATCHLIST if c["status"] == "Considering"),
            "status": "ok"
        }
...
def calculate_ipo_score(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if data.get("vix_is_extreme_low"):
        ...
        score += 25
        signals.append({"level": "RED", "msg": f"⚠️ VIX {vix:.1f} 극단적 저점 — IPO 흥행 마지막 조건 충족"})
